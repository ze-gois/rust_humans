...
pub const DEFAULT_BPM: f32 = 108.0;
...
/// user edits it.
pub const DEFAULT_MELODY: [i32; 32] = [
...
pub fn render_notes(sample_rate: u32, bpm: f32, melody: &[i32]) -> Vec<f32> {
    ...
    let samples_per_note = ample::math::roundf(sample_rate * note_seconds) as usize;
    ...
    for (note_index, semitone) in melody.iter().enumerate() {
        ...
        for i in 0..samples_per_note {
            ...
            let vibrato = 1.0 + 0.004 * ample::math::sinf(TAU * 5.0 * absolute_t);

            let fundamental = ample::math::sinf(TAU * frequency * vibrato * t);
            let overtone = 0.35 * ample::math::sinf(TAU * harmonic * vibrato * t);
...
pub fn frequency_for_semitone(semitone_from_a4: i32) -> f32 {
    440.0 * ample::math::powf(2.0, semitone_from_a4 as f32 / 12.0)
}
